...
+ (NSArray*)handleColorsForSimplyMusic:(int)theme {
    NSArray *array;
    UIColor *zero;
    UIColor *one;
    UIColor *two;
    ...
    switch (theme) {
        case 0:
            //70s
            //background
            zero = UIColorFromHex(0x5E412F);
            //font and blank waveform
            one = UIColorFromHex(0xFCEBB6);
            //button background
            two = UIColorFromHex(0xF0A830);
            //waveform fill color
            three = UIColorFromHex(0x78C0A8);
            //other accents
            four = UIColorFromHex(0xF07818);
        ...
        case 1:
            //flat design #2
            //background
            zero = UIColorFromHex(0x60646D);
            //font and blank waveform
            one = UIColorFromHex(0xFFFFF7); //flat black
            //transport button background
            two = UIColorFromHex(0x6B9262);
            //waveform fill color
            three = UIColorFromHex(0xFF6766);
            //top button background and toggle on
            four = UIColorFromHex(0x5B8BB0);
...
+ (NSString*)getGlobalFont {
    return nnKitGlobalFont;
}
+ (CGFloat)fontSize:(int)size {
...
+ (void)animatePulse:(CALayer *)layer {
    [CATransaction begin];
    ...
    a.toValue=[NSNumber numberWithFloat:.96];
    [layer addAnimation:a forKey:@"animateScale"];
    [CATransaction commit];
}

+ (void)animatePulse:(CALayer *)layer shrinkTo:(CGFloat)shrink withDuration:(CGFloat)dur {
    [CATransaction begin];
...
+ (void)addCloseViewTo:(UIView*)view withAlpha:(CGFloat)alpha withCloseMethod:(NSString *)selector fromClass:(id)class {
    UIView *closeView = [[UIView alloc] initWithFrame:[UIScreen mainScreen].bounds];
    closeView.tag = (int)999999999999999;
    UITapGestureRecognizer *tapCloseView = [[UITapGestureRecognizer alloc] initWithTarget:class action:NSSelectorFromString(selector)];
    [closeView addGestureRecognizer:tapCloseView];
    [nnKit addBlurToView:closeView isDark:YES withAlpha:alpha];
...
+ (void)dismissCloseViewFrom:(UIView*)view {
    for (UIView *closeView in view.subviews) {
        if (closeView.tag == (int)999999999999999) {
...
*/
    ...
    switch (selection) {
        ...
        case 8: //http://flatcolors.net/palette/948-retro-70s
            ...
            color1 = UIColorFromHex(0x5E4100);
